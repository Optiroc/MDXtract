...
class PMD():
  def __init__(self, data):
    ...
    data_offsets = []
    for i in range(12):
      data_offsets.append(get_uint16(data, 2 + i * 2, True))

    # Voice data
    voice_len = 0x1A
    voice_offset = data_offsets[11] + 1
    while True:
      if voice_offset + voice_len > len(data): break
      self.voice_data.append(data[voice_offset : voice_offset + voice_len])
      voice_offset += voice_len
      if voice_offset >= len(data): break
      if data[voice_offset] == 0x00: break
    ...
    if meta_offset + 1 >= len(data) or data[meta_offset] != 0xFF: return # No metadata
    ...
    for i,b in enumerate(meta_bin):
      try:
        ...
        if dec: self.meta_data[meta_keys[i]] = dec
