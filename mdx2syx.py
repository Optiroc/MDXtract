...
class MDX(object):
  def __init__(self, data):
    ...
    self.channels = (get_uint16(data, data_start + 2, True) >> 1) - 1

    # Get voice data
    voice_blob = data[get_uint16(data, data_start, True) + data_start : len(data)]
    voice_len = 0x1B
    voice_offset = 0
    while True:
    ...
    for i in range(self.channels):
      mml_offsets.append(get_uint16(data, data_start + 2 + (i * 2), True) + data_start)
    mml_offsets.append(get_uint16(data, data_start, True))
